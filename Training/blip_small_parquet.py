...
# Else, evaluate
    ...
    eval_ds = list(islice(ds, 10000, 10100))
    eval_dataset = [preprocess(sample) for sample in eval_ds]

    # Get inputs for Trainer prediction (only pixel_values)
    inputs = [{"pixel_values": item["pixel_values"]} for item in eval_dataset]

    # Get true text captions
    reference_ids = [item["input_ids"] for item in eval_dataset]
    references = processor.tokenizer.batch_decode(reference_ids, skip_special_tokens=True)
